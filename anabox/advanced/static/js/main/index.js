...
$(document).ready(function () {
    if($("#userId").val() == "" || $("#userId").val() == "None"){
       $("#todo-input-text").attr("readonly", true);
       $("#chatbot-input-text").attr("readonly", true);
    }
    ...
    function containerList() {
        $.ajax({
            url: "/container/containerListAjax",
            method: "GET",
            data: formData,
            dataType: "json",
    ...
    function recentBookingApply() {
        $.ajax({
            url: "/recentBookingApply",
            method: "GET",
            data: {},
            dataType: 'json',
            success: function(data) {
                ...
                data.forEach(row => {
                    rowsHtml += `
                        <tr>
                            ...
                            <td style="text-align:left">${Number(row[2]).toLocaleString()}원</td>
                        </tr>
                    `;
                });

                document.getElementById("recentBookingApply").innerHTML = rowsHtml;
            },
            error: function(request) {
                const message = request.responseJSON?.message || "서버 요청 중 오류가 발생했습니다!";
                swal(message);
